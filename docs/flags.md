...
- `hunt_weak_requires_psk` (Schwelle `hunt_weak_snr_db`, Default −13): schwächere Rufer werden nur angerufen, wenn PSK Reporter sie in `psk_heard_us` führt. **Widerlegt und abgeschaltet am 21.09.2026** (Default jetzt `false`, ebenso `hunt_weak_requires_psk_ab`): Das A/B über 14 Tage ergab 165 QSOs aus 1007 Anrufen mit Filter gegen 163 aus 941 ohne (z = −0,12), und im Schattenprotokoll des Kontrollarms schlossen die 83 verhinderten Ziele zu 16,9 % ab — gegen 16,0 % ohne Gate. Der ursprüngliche Beleg (unter −13 dB nur 3 %) war eine Quote je Anruf ohne Vergleichsgruppe und maß mit, dass schwache Ziele meist alternativlos sind (813-mal gab der Filter selbst nach). Siehe `analyse/regelregister.py`.
