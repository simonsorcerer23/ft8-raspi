...
| `chrony.service` | time |
| `ft8-ap-fallback.service` + `ft8-hostapd.service` | AP fallback — started by the controller's watchdog after `network.fallback_delay_s` without any upstream link, or by hand from the Wi-Fi page |
...
- **FT4 decode + encode** in parallel (7.5 s slot, 4-FSK, 105 symbols) —
  ...
  FT8↔FT4 switches **live on hot-reload** (no restart): `SlotClock.set_slot_seconds`
  retunes the running clock, the `_iter` loop re-reads the cadence each cycle
  (fixed 2026-06-11 — previously the running iterator kept the old tempo).
...
| Decoder mode mix | standard / deep / multi / extreme (default extreme since v0.7.1). Two-stage since v0.68 (stage 1 standard decides TX, stage 2 in a thread); on the Pi 5 single-stage since v0.84.x — the full mode decides for itself (`decoder_late_pass: false`) | only 1 fixed mode | on the Pi 4B the whole extreme mode put the TX start 2.8 s after the boundary, two-stage 0.3–0.4 s. Since the candidate loops use all cores, extreme takes 0.2–0.7 s live on the Pi 5 and fits ahead of the transmit decision — the ~37 % of decodes that previously arrived too late are now answerable. Fallback after three late TX starts: back to two-stage first, then to standard |
...
| Third decoder stage | WSJT-X's own `jt9` (package `wsjtx`) run as a subprocess on the slot WAV (`/dev/shm`), depth 2, parallel to stage 2, results via the late-ingest path | port the Fortran decoder | GPL stays outside our MIT code; on the Pi 4B 6 s (max 9 s) per slot at depth 2, 97.5 % of WSJT-X's decodes alone, 98.3 % together with our decoder plus ~50 decodes the reference lacks. Depth 3 cost 11–17 s on the 4B and was impossible there; on the Pi 5 it is 4.3 s on the corpus and ~5.8 s live, permanently on since 2026-09-09 |
