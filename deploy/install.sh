...
#
# Idempotent. Re-runnable. Reads /opt/ft8-appliance/architecture.md for
# context, /etc/ft8-appliance/config.yaml for runtime config.
#
# Expected starting point:
#   * Pi OS Lite 64-bit (bookworm) booted from NVMe
#   * User `pi` with sudo
#   * Repo synced to /opt/ft8-appliance/
#   * SSH key from workstation installed
...
if [ ! -d "${APP_DIR}" ]; then
    echo "expected ${APP_DIR} to exist (rsync the repo there first)" >&2
    exit 1
fi
...
install -d -o pi -g pi "${ETC_DIR}" "${LOG_DIR}" "${LIB_DIR}"
install -d -o pi -g pi "${APP_DIR}/tiles"
...
sudo -u pi "${APP_DIR}/backend/.venv/bin/python" -c "
