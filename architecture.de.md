...
| Decoder-Mode-Mix | standard / deep / multi / extreme (Default extreme seit v0.7.1). Zweistufig seit v0.68 (Stufe 1 standard entscheidet über TX, Stufe 2 im Thread); auf dem Pi 5 seit v0.84.x einstufig — der volle Modus entscheidet selbst (`decoder_late_pass: false`) | nur 1 fixer Mode | am Pi 4B lag der Sendestart mit dem ganzen extreme-Modus 2,8 s nach der Slotgrenze, zweistufig 0,3–0,4 s. Seit die Kandidatenschleifen alle Kerne nutzen, braucht extreme am Pi 5 live 0,2–0,7 s und passt vor die Sendeentscheidung — die vorher ~37 % zu spät gefundenen Decodes sind damit beantwortbar. Rückzug bei dreimal zu spätem Sendestart: erst zurück in die Zweiteilung, dann auf standard |
...
| Dritte Decoderstufe | WSJT-X' eigener `jt9` (Paket `wsjtx`) als Unterprozess auf der Slot-WAV (`/dev/shm`), Tiefe 2, parallel zu Stufe 2, Ergebnisse über den Nachreich-Pfad | den Fortran-Decoder portieren | GPL bleibt außerhalb unseres MIT-Codes; am Pi 4B 6 s (max 9 s) pro Slot bei Tiefe 2, allein 97,5 % der WSJT-X-Decodes, zusammen mit unserem Decoder 98,3 % plus ~50 Decodes, die die Referenz nicht hat. Tiefe 3 kostete am 4B 11–17 s und war dort unmöglich; am Pi 5 sind es 4,3 s im Korpus und ~5,8 s live, seit 2026-09-09 dauerhaft aktiv |
